#include <iostream>
#include <cstdio>
#include <cstdlib>
#include <cstring>
using namespace std;

unsigned int rc[15]={
    0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36
};

unsigned char s[16][16] = {};
unsigned int t[4][16][16];
unsigned char trans[4][4]={
    {0x0e,0x09,0x0d,0x0b},
    {0x0b,0x0e,0x09,0x0d},
    {0x0d,0x0b,0x0e,0x09},
    {0x09,0x0d,0x0b,0x0e}
};



unsigned char gls(unsigned int a,unsigned int b){

    unsigned int ans=0,mod=0x8d800000,chk=0x80000000;
    while(a){
        if(a&1) ans^=b;
        a=a>>1;
        b=b<<1;
    }
    while(mod!=0x8d){
        if(ans&chk) ans^=mod;
        chk=chk>>1;
        mod=mod>>1;
    }
    return ans;
}

int main()
{
    freopen("tbox3.h","w",stdout);
    for(int i=0;i<4;i++)
        for(int j=0;j<16;j++)
            for(int k=0;k<16;k++){
                for(int l=0;l<4;l++){
                    t[i][j][k]<<=8;
                    t[i][j][k]|=gls(trans[i][l],s[j][k]);
                }
            }

    printf("unsigned int tinv[4][16][16]={\n");
    for(int i=0;i<4;i++){
        printf("{");
        for(int j=0;j<16;j++){
            printf("{");
            for(int k=0;k<16;k++){
                printf("0x%08x",t[i][j][k]);
                if(k!=15) printf(", ");
            }
            if(j!=15) printf("},\n");
            else printf("}\n");
        }
        if(i!=3) printf("},\n");
        else printf("}\n");
    }
    printf("};\n");
}